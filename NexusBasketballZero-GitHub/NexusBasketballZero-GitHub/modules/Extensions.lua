...
do
    ...
    local config = {
        ...
        PlayerFilter = "Все", MaxDistance = 600, BallESP = false, HoopESP = false,
        Aim = false, AimTarget = "Кольцо", Hoop = "Авто (по команде)",
        AimRadius = 250, Smoothing = 10, Prediction = 0, AimHeld = false,
        ShowFOV = false, Trajectory = false, Horizon = 1.5,
        Theme = "Фиолетовая",
        NoDribbleCD = false, AlwaysRun = false, FaceHoop = false,
        ActionDistance = 12, ActionInterval = 0.8,
        PerfectShot = false,
        ...
        SmartPass = false, PassMode = "Свободный партнёр", PassPlayer = "Не выбран",
        PressurePass = false, PassDistance = 120, AutoRebound = false, ReboundRange = 65,
        AntiSlow = false, CustomShiftLock = false, Shoulder = "Правое",
    ...
    local controllers, controllerState, controllerTask = {}, "не подключены", nil
    local sharedTables = {}
    ...
    -- Меняем метод только конкретного объекта, не глобальный метаметод Roblox.
    local methodPatches = {}
    ...
    local function makeDribbleWrapper(original, active, beforeCall)
        return function(self, ...)
            if active() and type(self) == "table" and not table.isfrozen(self) then beforeCall(self) end
    ...
    local function connectControllers()
        if controllerTask or controllerState == "подключены" then return end
        controllerState = "автопоиск…"
        controllerTask = task.defer(function()
            if type(getgc) ~= "function" then
                controllerState = "getgc недоступен"
                controllerTask = nil
                return
            end
            local signatures = {
    ...
    local function marker(key)
        ...
        local text = make("TextLabel", {BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5, 1),
            Position = UDim2.new(0.5, 0, 0, -3), Size = UDim2.fromOffset(280, 56), TextColor3 = palette.Text,
            TextWrapped = true, TextStrokeTransparency = 0.3, Font = Enum.Font.GothamMedium, TextSize = 12}, box)
    ...
    local function selectedHoop()
        ...
        if config.Hoop == "Авто (по команде)" and player.Team then
    ...
    local function passesFilter(other)
        return config.PlayerFilter == "Все" or (config.PlayerFilter == "Команда" and sameTeam(other))
            or (config.PlayerFilter == "Соперники" and not sameTeam(other))
    end
    ...
    local function updateZoneModifiers()
        ...
        if not zones or not zone or zone == "" then
            stealNote.Text = config.NoStealCD and "No CD Steal: таблица Zones ещё не найдена" or "No CD Steal выключен"
            return
        end
        ...
        if config.NoStealCD then
            ...
            stealNote.Text = type(values) == "table" and values[zone] == 0
                and "Steal CD = 0; уже начатый таймер сначала истечёт"
                or "No CD Steal: таблица недоступна для изменения"
        else stealNote.Text = "No CD Steal выключен" end
    end
    ...
    local function updateControllers(dt)
        ...
        if ballController then
            if config.NoDribbleCD then
                local installed = installMethodPatch("Dribble", ballController, "Dribble", function(original)
                    return makeDribbleWrapper(original, function() return alive and config.NoDribbleCD end, function(self)
                        tableOverride(self, "DribbleDB", tick() - 5)
                    end)
                end)
                dribblePatchStatus = installed and "готов" or "конфликт"
            else
                removeMethodPatch("Dribble"); restoreTable(ballController, "DribbleDB"); dribblePatchStatus = "OFF"
            end
        else
            removeMethodPatch("Dribble"); dribblePatchStatus = config.NoDribbleCD and "нет Knit" or "OFF"
        end
        config.PerfectShot = false
        removeMethodPatch("Shot")
        shotPatchStatus = "отключён после блокировки"
    ...
    extras = (function()
        ...
        local function notifyFailure(name)
            state.message = name .. ": обработчик недоступен"
        end
        ...
        local function choosePass()
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                local r = partOf(p)
                if p ~= player and sameTeam(p) and r and (config.PassMode ~= "Выбранный игрок" or p.Name == config.PassPlayer) then
                    ...
                    if distance > 1 and distance <= config.PassDistance and clearPath(r.Position, p.Character) then
                        ...
                        if config.PassMode == "Ближе к кольцу" and hoop then candidate = -(r.Position - hoop.Position).Magnitude end
            ...
            task.spawn(function()
                local success, err = pcall(function()
                    ...
                    if alive and pass.LookingAt == target.Character then
                        if config.SmartPass then pass.LookingAt = previous else restoreTable(pass, "LookingAt") end
                    end
                    if not fired then notifyFailure("Пас") end
                end)
                if not success then state.message = "Пас: " .. tostring(err) end
        ...
        function api.update(dt)
            ...
            local automation = config.SmartPass or config.PressurePass or config.AutoRebound or state.requestPass
            if not automation then
                ...
                info.Text = "Автоматика выключена · подписок BallService нет"
                return
            end
            info.Text = state.message or "Автоматика: пас / подбор"
            ...
            local ready = root and humanoid and humanoid.Health > 0 and type(values) == "table" and ballController
                ...
                and camera and camera.CameraType ~= Enum.CameraType.Scriptable and not Input:GetFocusedTextBox()
            if not ready then
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                local er = partOf(p)
                if opponent(p) and er then
                    local delta = root.Position - er.Position
                    if delta.Magnitude <= config.ActionDistance then
                        nearEnemy = true
                    end
                end
            end
            local pass = controllers.PassController
            if pass and not state.passPending then
                if config.SmartPass and values.HasBall then
                    local target = choosePass()
                    tableOverride(pass, "LookingAt", target and target.Character or nil)
                else restoreTable(pass, "LookingAt") end
            end
            local manualPass = state.requestPass; state.requestPass = false
            if values.HasBall and (manualPass or (config.PressurePass and nearEnemy and now >= state.nextAction)) then
        ...
        function api.camera(dt, camera)
            ...
            local active = config.CustomShiftLock and root and humanoid and humanoid.Health > 0
                and camera.CameraType ~= Enum.CameraType.Scriptable and not Input:GetFocusedTextBox()
            if active then
                ...
                Input.MouseBehavior = Enum.MouseBehavior.LockCenter; Input.MouseIconEnabled = false
                installMethodPatch("ShiftHead", controllers.CameraController, "HeadMovement", function()
                    return function() end
                end)
                override(humanoid, "CameraOffset", Vector3.new(config.ShoulderOffset * (config.Shoulder == "Левое" and -1 or 1), config.CameraHeight, 0))
                local movement = controllers.MovementController
                local s = movement and movement.States or {}
                if not (config.FaceHoop or config.Fly or s.Shooting or s.Dunking or s.Ability or s.Stunned) then
                ...
                if state.shiftOwned then
                    ...
                    state.shiftOwned = false
                    if humanoid and not config.FaceHoop and not config.Fly then restore(humanoid, "AutoRotate") end
                end
            end
        end
    ...
    Tabs.Aim:AddSection("Camera Aim")
    csToggle("CS_Aim", "Aim", Tabs.Aim, "Camera Aim", "Aim the camera at a target")
    csToggle("CS_AimHeld", "AimHeld", Tabs.Aim, "Aim While Held", "Only aim while holding RMB")
    local aimTargetDd = Tabs.Aim:AddDropdown("CS_AimTarget", {
        Title = "Target",
        Default = "Кольцо",
        Values = {"Кольцо", "Мяч", "Ближайший соперник", "Владелец мяча"},
        Callback = function(Value) config.AimTarget = Value end,
    })
    local hoopDd = Tabs.Aim:AddDropdown("CS_Hoop", {
        Title = "Hoop",
        Default = "Авто (по команде)",
        Values = {"Авто (по команде)", "Home", "Away", "Ближайшее"},
        Callback = function(Value) config.Hoop = Value end,
    })
    csSlider("CS_Smoothing", "Smoothing", Tabs.Aim, "Smoothing", "Higher is smoother", 1, 30, 1, 1)
    csSlider("CS_Prediction", "Prediction", Tabs.Aim, "Prediction", "Prediction time in seconds", 0, 0.5, 0.01, 2)
    csSlider("CS_AimRadius", "AimRadius", Tabs.Aim, "Aim Radius", "Aim radius in px", 30, 800, 5, 1)
    csToggle("CS_ShowFOV", "ShowFOV", Tabs.Aim, "Show FOV Circle", "Draw the aim radius circle")
    ...
    Tabs.Zone:AddButton({
        Title = "Сбросить все функции",
        Description = "Off all merged courtside features",
        Callback = function()
            for key, value in pairs(config) do if type(value) == "boolean" then config[key] = false end end
            for _, id in ipairs(csToggleIds) do
                local option = NexusUI.Options[id]
                if option and option.Value then
                    pcall(function() option:SetValue(false) end)
                end
            end
            stopFly(); restoreAll(); restoreGameChanges()
        end,
    })
    Tabs.Zone:AddSection("Game Controllers")
    csToggle("CS_NoDribbleCD", "NoDribbleCD", Tabs.Zone, "No Dribble CD", "Remove local dribble cooldown")
    csToggle("CS_NoStealCD", "NoStealCD", Tabs.Zone, "No CD Steal", "Remove local stealth cooldown")
    ...
    csToggle("CS_FaceHoop", "FaceHoop", Tabs.Zone, "Face Hoop", "Turn the character towards the hoop")
    csSlider("CS_ActionDistance", "ActionDistance", Tabs.Zone, "Auto Action Radius", "Radius for automatic actions", 3, 25, 1, 1)
    ...
    csToggle("CS_SmartPass", "SmartPass", Tabs.Zone, "Smart Pass", "Automatically pick the receiver")
    local passModeDd = Tabs.Zone:AddDropdown("CS_PassMode", {
        Title = "Receiver Mode",
        Default = "Свободный партнёр",
        Values = {"Свободный партнёр", "Выбранный игрок", "Ближе к кольцу"},
        Callback = function(Value) config.PassMode = Value end,
    })
    local function refreshPassPlayer()
        local names = {"Не выбран"}
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= player and sameTeam(p) then table.insert(names, p.Name) end
        end
        if #names > 1 then
            table.sort(names, function(a, b) return a == "Не выбран" or (b ~= "Не выбран" and a < b) end)
        end
    ...
    refreshPassPlayer()
    csToggle("CS_PressurePass", "PressurePass", Tabs.Zone, "Pressure Pass", "Auto pass while pressured")
    csSlider("CS_PassDistance", "PassDistance", Tabs.Zone, "Max Pass Distance", "Maximum pass distance", 10, 180, 5, 1)
    Tabs.Zone:AddButton({
        Title = "Отдать пас сейчас",
    ...
    local themeDd = Tabs.Visuals:AddDropdown("CS_Theme", {
        Title = "Accent",
        Default = "Фиолетовая",
        Values = {"Фиолетовая", "Бирюзовая", "Оранжевая"},
        Callback = function(Value) config.Theme = Value end,
    })
    ...
    Run:BindToRenderStep(renderName, Enum.RenderPriority.Camera.Value + 10, function(dt)
        ...
        if root and humanoid and humanoid.Health > 0 then
            if config.FaceHoop then
                local hoop = selectedHoop()
                if hoop then
                    local point = Vector3.new(hoop.Position.X, root.Position.Y, hoop.Position.Z)
                    if (point - root.Position).Magnitude > 0.01 then
                        override(humanoid, "AutoRotate", false)
                        root.CFrame = root.CFrame:Lerp(CFrame.lookAt(root.Position, point), 1 - math.exp(-dt * 35 / config.Smoothing))
                    end
                end
            elseif not config.Fly then restore(humanoid, "AutoRotate") end
        ...
        if theme ~= config.Theme then
            theme = config.Theme
            accent = theme == "Бирюзовая" and Color3.fromRGB(80, 215, 186)
                or theme == "Оранжевая" and Color3.fromRGB(255, 163, 87) or Color3.fromRGB(151, 119, 255)
        ...
        if config.Aim and not Input:GetFocusedTextBox() and (not config.AimHeld or Input:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)) then
            if config.AimTarget == "Кольцо" then target = selectedHoop()
            elseif config.AimTarget == "Мяч" then target = ball
            elseif config.AimTarget == "Владелец мяча" then
        ...
        if config.BallESP then markPoint("Ball", ball, "МЯЧ", camera) end
        if config.BallOwnerESP then
            local owner = ballOwner()
            local part = owner and owner.Character and owner.Character:FindFirstChild("HumanoidRootPart")
            markPoint("BallOwner", part, owner and ("С МЯЧОМ: " .. owner.DisplayName) or "", camera)
        end
        if config.HoopESP then
            markPoint("Home", hoopPart("Home"), "КОЛЬЦО HOME", camera)
            markPoint("Away", hoopPart("Away"), "КОЛЬЦО AWAY", camera)
        end
