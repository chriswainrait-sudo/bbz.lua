...
local function getAutoSpinStylesDependencies()
    ...
    local service = knit.GetService("StyleService")
    local network = knit.GetController("Network")
    ...
    assert(service and service.Spin and type(service.Spin.Fire) == "function", "StyleService.Spin is unavailable")
    ...
    Callback = function(Value)
        ...
        autoSpinStylesThread = task.spawn(function()
            ...
            while autoSpinStylesEnabled and runId == autoSpinStylesRun do
                ...
                local data = dependencies.Network and dependencies.Network.Player
                if not styleValue or type(data) ~= "table" then
                    task.wait(0.5)
                    continue
                end
                if isAutoSpinStylesTarget(dependencies, styleValue.Value) then break end
                if data.LimitedStyleEquipped ~= nil then break end

                local lockedSlots = data.LockedSlots
                local styleLocks = type(lockedSlots) == "table" and lockedSlots.Style
                if type(styleLocks) == "table" and styleLocks[data.StyleSlotEquipped] then break end

                local spins, money = tonumber(data.Spins), tonumber(data.Money)
                if spins and money and spins <= 0 and money < 2000 then break end
                if LocalPlayer.Team ~= game.Teams:FindFirstChild("Visitor") then
                    task.wait(0.5)
                    continue
                end

                local beforeStyle = styleValue.Value
                local beforeSpins = data.Spins
                local beforeMoney = data.Money
                local beforePity = data.StyleSpinsDone
                local sentAt = os.clock()
                local fired = pcall(function()
                    dependencies.Service.Spin:Fire(false, "FroggyWoggy")
                end)
                if not fired then
                    autoSpinStylesDependencies = nil
                    dependencies = nil
                    break
                end
                ...
                repeat
                    ...
                    data = dependencies.Network and dependencies.Network.Player
                    if isAutoSpinStylesTarget(dependencies, styleValue.Value) then
                        accepted = true
                        break
                    end
                    accepted = styleValue.Value ~= beforeStyle
                        or type(data) == "table" and (data.Spins ~= beforeSpins
                            or data.Money ~= beforeMoney or data.StyleSpinsDone ~= beforePity)
                until accepted or os.clock() - sentAt >= 12

                if isAutoSpinStylesTarget(dependencies, styleValue.Value) then break end
                if accepted then
                    local cooldown = 10.1 - (os.clock() - sentAt)
                    if cooldown > 0 then task.wait(cooldown) end
                else
                    task.wait(1.5)
                end
            end
...
Tabs.Main:AddToggle("AutoDribble", {
    Title = "Auto Dribble",
    Description = "Instantly dribbles when a nearby enemy attempts to steal",
    Default = false,
    Callback = function(Value)
        ...
        AutoDribbleConnection = ballService.Steal:Connect(function(stealer)
            if not AutoDribbleEnabled or not stealer or stealer == LocalPlayer.Character then return end

            local player = Players:GetPlayerFromCharacter(stealer)
            if player and player.Team == LocalPlayer.Team then return end

            local character = LocalPlayer.Character
            local root = character and character:FindFirstChild("HumanoidRootPart")
            local enemyRoot = stealer:FindFirstChild("HumanoidRootPart")
            if not root or not enemyRoot or (root.Position - enemyRoot.Position).Magnitude > range then return end

            pcall(ballController.Dribble, ballController)
        end)
    end
})
...
Tabs.Main:AddSection("Additional", "Left")
