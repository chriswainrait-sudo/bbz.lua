...
Tabs.Main:AddToggle("AutoDribble", {
    Title = "Smart Auto Dribble",
    Description = "Dribbles when a nearby enemy attempts to steal",
    Default = false,
    Callback = function(Value)
        ...
        local animations = {
            ["rbxassetid://132607768946898"] = true,
            ["rbxassetid://106268822474526"] = true,
        }
        local connections = {}
        ...
        local function activate(player, character, track)
            if not AutoDribbleEnabled or not localPlayerHasBall() or player.Team == LocalPlayer.Team then return end
            local animationId = track.Animation and track.Animation.AnimationId
            if not animations[animationId] then return end
            ...
            if not root or not enemyRoot or (root.Position - enemyRoot.Position).Magnitude > 100 then return end

            local now = os.clock()
            if now - lastActivation < 0.1 then return end
            local mobile = PlayerGui:FindFirstChild("Mobile")
            local ball = mobile and mobile:FindFirstChild("Ball")
            local button = findGuiButton("Dribble", ball and ball:FindFirstChild("Dribble"))
        ...
        local function connectCharacter(player, character)
            ...
            if animator then
                add(animator.AnimationPlayed:Connect(function(track)
                    activate(player, character, track)
                end))
            end
        end
        ...
        local function connectPlayer(player)
            ...
            if player.Character then task.spawn(connectCharacter, player, player.Character) end
        ...
        add(Players.PlayerAdded:Connect(connectPlayer))

        AutoDribbleConnection = {
            Disconnect = function()
