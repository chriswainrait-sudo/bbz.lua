...
function NexusUI:CreateWindow(options)
    ...
    subtitleGradient.Rotation = 15
    subtitleGradient.Offset = Vector2.new(-1, 0)
    subtitleGradient.Parent = logoSubtitle
    NexusUI:BindColor(logoSubtitle, "TextColor3", function() return Color3.new(1, 1, 1) end)
    local function silverPalette()
        if not subtitleGradient.Parent then return end
        local light = NexusUI.ThemeName == "Light"
        local base = light and Color3.fromRGB(65, 74, 89) or Color3.fromRGB(176, 184, 199)
        local shine = light and Color3.fromRGB(108, 121, 140) or Color3.fromRGB(255, 255, 255)
        subtitleGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, base), ColorSequenceKeypoint.new(0.35, base),
            ColorSequenceKeypoint.new(0.5, shine), ColorSequenceKeypoint.new(0.65, base),
            ColorSequenceKeypoint.new(1, base)
        })
    end
    silverPalette()
    table.insert(NexusUI.ThemeRefresh, silverPalette)
    local silverTween = TweenService:Create(subtitleGradient,
        TweenInfo.new(3.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, false, 0.6),
        {Offset = Vector2.new(1, 0)})
    local function updateSilver()
        if mainFrame.Visible and screenGui.Enabled then silverTween:Play() else silverTween:Pause() end
    end
    TrackGuiConnection(mainFrame:GetPropertyChangedSignal("Visible"):Connect(updateSilver))
    TrackGuiConnection(screenGui:GetPropertyChangedSignal("Enabled"):Connect(updateSilver))
    RegisterUnloadCallback(function() silverTween:Cancel() end)
    updateSilver()
    ...
    function window:AddTab(tabData)
        ...
        function tabObject:AddDropdown(id, data)
            ...
            dropdownStroke.Transparency = 0.55
            ...
            local function rebuildOptions()
                ...
                for index, value in ipairs(dropdown.Values) do
                    ...
                    optionViews[value] = {option, optionLabel, selected, paint}
